...
const FUNNEL_DATA = [
  ...
  { stage: 'Allotted', count: 310, color: '#8b5cf6' },
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 bg-[#020617] -m-6 p-10 min-h-screen text-slate-200">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-3xl font-black tracking-tight uppercase text-white">Strategic Command Center</h2>
                    </div>
                    <p className="text-sm font-medium text-slate-400 flex items-center gap-2">
                         <Activity size={16} className="text-emerald-500" /> Real-time Global Business Intelligence Aggregator
                    </p>
                </div>
                <div className="flex gap-3">
                    <button className="flex items-center gap-2 px-6 py-3 bg-white text-black rounded-2xl text-[11px] font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-xl shadow-white/5">
                        <Download size={14} /> Board Report (PDF)
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                    { label: 'MTD Revenue', value: '₹7.24 Cr', change: '+12.4%', icon: <DollarSign />, color: 'text-emerald-400' },
                    { label: 'Net Profit Margin', value: '18.2%', change: '+2.1%', icon: <TrendingUp />, color: 'text-blue-400' },
                    { label: 'Inventory Value', value: '₹24.8 Cr', change: '-4.8%', icon: <Waves />, color: 'text-purple-400' },
                    { label: 'Market Share', value: '31.5%', change: '+1.2%', icon: <Target />, color: 'text-amber-400' },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-2 bg-slate-900/50 border border-slate-800 rounded-[40px] p-10 flex flex-col">
                    <div className="flex justify-between items-start mb-10">
                        <div>
                            <h4 className="text-xl font-black text-white uppercase tracking-tight">Revenue Trajectory</h4>
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Monthly Gross Revenue vs Margin Index</p>
                        </div>
                ...
                <div className="bg-[#0f172a] rounded-[40px] p-10 border border-slate-700 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="h-[300px] w-full relative z-10">
                        <ResponsiveContainer width="100%" height="100%">
                            <RadialBarChart cx="50%" cy="50%" innerRadius="20%" outerRadius="100%" barSize={15} data={GAUGE_DATA} startAngle={90} endAngle={450}>
                                <RadialBar background dataKey="value" cornerRadius={30} />
                                <Legend iconSize={10} layout="vertical" verticalAlign="bottom" wrapperStyle={{ bottom: 0, textTransform: 'uppercase', fontSize: '9px', fontWeight: '900' }} />
                            </RadialBarChart>
                        </ResponsiveContainer>
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span className="text-5xl font-black text-white">84%</span>
                            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mt-2">Target Pacing</span>
                        </div>
                    </div>

                    <div className="mt-12 pt-10 border-t border-slate-700 relative z-10">
                        <div className="flex justify-between items-center mb-6">
                             <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400">"What-If" Impact Analysis</h5>
                             <Dna size={16} className="text-blue-400 animate-spin-slow" />
                        </div>
                        <p className="text-[11px] text-slate-400 font-medium mb-10 leading-relaxed">
                            Adjusting the **Finance Payout Multiplier** to see potential bottom-line impact.
                        </p>
                        <input 
                            type="range" min="1" max="2" step="0.1" value={payoutMultiplier} onChange={(e) => setPayoutMultiplier(Number(e.target.value))}
                            className="w-full h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-blue-500 mb-6"
                        />
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/20 p-6 rounded-[32px] flex items-center justify-between shadow-inner">
                            <div>
                                <p className="text-[10px] font-black uppercase text-blue-400">Proj. Profit Delta</p>
                                <p className="text-xl font-black text-white">+₹{estimatedProfitImpact.toLocaleString()}</p>
                            </div>
                            <div className="px-4 py-2 bg-blue-500 rounded-xl text-[9px] font-black uppercase tracking-widest">Apply</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <div className="bg-slate-900/50 border border-slate-800 rounded-[40px] p-10 shadow-sm overflow-hidden relative group">
                    ...
                    <h4 className="text-xl font-black text-white uppercase tracking-tight mb-10 relative z-10">Unit Pipeline Funnel</h4>
                    ...
                    <div className="mt-6 flex justify-between px-4 border-t border-slate-800 pt-8 relative z-10">
                         <div className="text-center">
                            <p className="text-[10px] font-black uppercase text-slate-500">Inventory Turnover</p>
                            <p className="text-2xl font-black text-white">12.4 Days</p>
                         </div>
                         <div className="text-center">
                            <p className="text-[10px] font-black uppercase text-slate-500">Pipeline Value</p>
                            <p className="text-2xl font-black text-white">₹18.5 Cr</p>
                         </div>
                         <div className="text-center">
                            <p className="text-[10px] font-black uppercase text-slate-500">Closure Rate</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-slate-900/50 border border-slate-800 p-10 rounded-[40px] shadow-sm flex flex-col justify-between overflow-hidden relative">
                         ...
                         <div className="relative z-10">
                             <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400 mb-6">Approval Gates</h5>
                             <div className="space-y-4">
                                 {[
                                    { id: 'APP-99', req: 'HS Discount', value: '₹1,45,000', by: 'SM Rahul' },
                                    { id: 'APP-102', req: 'CAPEX Request', value: '₹4,12,000', by: 'Acc Gupta' },
                                 ].map((app, i) => (
                    ...
                    <div className="bg-emerald-500 p-10 rounded-[40px] text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden group">
                        ...
                        <div className="relative z-10 h-full flex flex-col justify-between">
                            <div>
                                <h5 className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-8">Quarterly Net Revenue</h5>
                                ...
                                <p className="text-[11px] font-bold opacity-80 mt-2 leading-relaxed">
                                    Showroom efficiency is at an all-time high of **94.2%** this fiscal quarter.
                                </p>
                            </div>
                            <div className="pt-10 border-t border-white/20 mt-10">
                                <div className="flex items-center gap-3">
                                    <Activity size={20} className="text-white animate-pulse" />
                                    <span className="text-[10px] font-black uppercase tracking-widest">Global Status: Positive</span>
                                </div>
                            </div>
                        </div>
                    </div>
