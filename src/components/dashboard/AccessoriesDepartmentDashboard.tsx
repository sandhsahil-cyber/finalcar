    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricsCard
                    title="Total Fittings Sale" value={formatCurrency(totalAccessoryRevenue)} subtitle="Current Month Revenue"
                    icon={<ShoppingBag className="w-5 h-5" />} color="#f43f5e"
                />
                <MetricsCard
                    title="Wait for Fitting" value={String(pendingFittings).padStart(2, '0')} subtitle="Cars in Work Area"
                    icon={<Wrench className="w-5 h-5" />} color="#f59e0b"
                />
                <MetricsCard
                    title="Ready for Delivery" value={String(completedFittings).padStart(2, '0')} subtitle="Fitting Done"
                    icon={<CheckCircle2 className="w-5 h-5" />} color="#10b981"
                />
                <MetricsCard
                    title="Average per Car" value={formatCurrency(totalAccessoryRevenue / (deals.filter(d => d.accessoriesAmount).length || 1))} subtitle="Work Speed"
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
                ...
                <div className="xl:col-span-3 bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
                    <div className="p-6 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            <h3 className="text-lg font-bold text-gray-900">Car Fitting List</h3>
                            <p className="text-xs text-gray-500 font-medium">Track fitting work and parts stock</p>
                        </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-gray-50/50 text-[10px] uppercase tracking-widest font-bold text-gray-400">
                                <tr>
                                    <th className="px-6 py-4">Car & Owner Name</th>
                                    <th className="px-6 py-4">Fitting Items</th>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col items-center">
                                                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${item.fitmentStatus === 'In Progress' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'
                                                    ...
                                                    {item.fitmentStatus === 'In Progress' ? 'Doing Work' : 'Work Done'}
                                                </span>
                                                {item.pendingItems.length > 0 && (
                                                    <p className="text-[9px] text-red-500 font-bold mt-1">Parts Waiting: {item.pendingItems[0]}</p>
                                                )}
                                            </div>
                                        </td>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-6 relative">
                        <h4 className="font-bold text-gray-900 flex items-center gap-2 mb-4">
                            <CheckSquare className="w-5 h-5 text-rose-500" /> Fitting Work Card
                        </h4>
                        ...
                        {selectedFitment ? (
                            <div className="space-y-4 animate-in zoom-in-95 duration-200">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] text-gray-500 font-bold uppercase">Fitting Person</label>
                                ...
                                <div className="p-4 bg-rose-50/50 border border-rose-100 rounded-2xl">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        ...
                                        <span className="text-xs font-bold text-rose-700 uppercase">Work Done & Checked</span>
                                    </label>
                                </div>
                                ...
                                <div className="space-y-2">
                                    <button className="w-full py-3 bg-gray-900 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-black transition-all">
                                        <Upload className="w-4 h-4" /> Save Fitting Bill
                                    </button>
                                    <button
                                        onClick={() => { 
                                            updateDepartmentStatus(selectedFitment.id, 'Accessories', 'Completed');
                                            alert('Sales Person told: Fitting Complete!'); 
                    ...
                    <div className="bg-gray-900 rounded-3xl p-6 text-white">
                        <div className="flex items-center justify-between mb-4">
                            <h4 className="font-bold text-sm">Part Stock Check</h4>
