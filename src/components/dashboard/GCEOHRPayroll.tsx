  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 bg-[#0f172a] -m-6 p-10 min-h-screen text-slate-200">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-3xl font-black tracking-tight uppercase text-white">Staff & Salary</h2>
          <p className="text-sm font-medium text-slate-400 flex items-center gap-2">
            <UserCheck size={16} className="text-blue-500" /> See all staff details and salary info
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {[
              { label: 'Total Staff', value: '1,242', change: '+14 New', icon: <Users />, color: 'blue' },
              { label: 'Monthly Salary Cost', value: '₹2.84 Cr', change: '+4.2%', icon: <DollarSign />, color: 'emerald' },
              { label: 'Incentive Paid', value: '₹1.15 Cr', change: '-1.4% This Week', icon: <Briefcase />, color: 'amber' },
              { label: 'Hiring Open', value: '42 Posts', change: '8 Urgent', icon: <Search />, color: 'purple' },
          ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          <div className="bg-slate-900 border border-slate-800 rounded-[40px] p-10 overflow-hidden relative group">
              <h4 className="text-xl font-black text-white uppercase tracking-tight mb-10">Brand-wise Incentive Paid (Lakhs)</h4>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-[40px] p-10 space-y-8">
               <h4 className="text-xl font-black text-white uppercase tracking-tight mb-4">Staff Distribution</h4>
                    ...
                    {PAYROLL_DATA.map((brand, i) => (
                        <div key={i} className="p-6 bg-slate-800/40 border border-slate-700/50 rounded-3xl hover:border-blue-500/20 transition-all flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-slate-500 uppercase mt-1">Performance Score: {brand.efficiency}%</p>
                            </div>
                            <div className="text-right">
                                <p className="text-xl font-black text-white tracking-tighter">{brand.headcount}</p>
                                <p className="text-[9px] font-black text-slate-500 uppercase">Staff Count</p>
                            </div>
                        </div>
                    ))}
               </div>
